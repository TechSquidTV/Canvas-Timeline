...
- [Media adapters](/docs/media-adapters) — Resolve source inputs, proxies, fallbacks, lazy loading, and external-clock transport.
...
adapter. Framework-neutral synchronization, source timing, lifecycle, operation,
and error contracts live in Core. Clips connect to media by stable
`clip.sourceId` values; the application resolves an original, proxy, cache, or
review choice and gives the adapter one matching source plus optional
content-equivalent load fallbacks.

The HTML adapter maps that registry to one native media element; Mediabunny
loads registered sources on demand for decoded frame/audio preview. Both package
roots are imperative and framework-free. Their React lifecycle and transport
hooks are published from explicit `./react` entry points. Start with
`useHTMLTimelineMedia` or `useMediabunnyTimelineMedia` in React, and read the
[media adapter guide](/docs/media-adapters) for proxy ownership, readiness,
preloading, timing, audio, and external-clock range behavior.
