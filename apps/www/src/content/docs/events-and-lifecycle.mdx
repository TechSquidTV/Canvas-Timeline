...
To retrieve active clips at a specific time, use `engine.media.getActiveLayers()` (or the React hook `useActiveLayers()`). This utility filters out hidden tracks, muted tracks, and disabled clips, then returns the results in your named layer buckets:
...
const clip = engine.geometry.getClip('clip1')?.clip;
...
The `TimelineEngine` publishes type-safe events for structural modifications, live interactions, and playback updates.
...
| **`edit:impacts`**     | Edit Preview      | Emitted when preview or live edit consequences change for renderer and custom guide affordances.         | `TimelineEditImpacts \| null`                    |
